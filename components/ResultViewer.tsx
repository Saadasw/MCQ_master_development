...
import { Download, Save, Layers, BookOpen, CheckSquare, CheckCircle2, Move, MousePointerClick, Loader2 } from 'lucide-react';
...
export const ResultViewer: React.FC<ResultViewerProps> = ({ originalImage, segments: initialSegments, subjects }) => {
  ...
  const handleSave = async (segment: QuestionSegment) => {
    if (savedIds.has(segment.id) || !segment.cropUrl) return;
    setIsSaving(true);
    try {
      // Use the new service
      await questionService.addQuestion(segment, segment.cropUrl);
  ...
  const handleSaveAll = async () => {
    const unsaved = segments.filter(s => !savedIds.has(s.id) && s.cropUrl);
    if (unsaved.length === 0) return;

    setIsSaving(true);
    try {
      // Save sequentially or parallel (parallel is faster but might hit rate limits if huge batch)
      await Promise.all(unsaved.map(s => questionService.addQuestion(s, s.cropUrl!)));
            ...
            {segments.map((seg) => {
              ...
              return (
                <div
                  ...
                  className={`absolute border-2 transition-colors cursor-pointer group ${isSelected
          ...
          {segments.map((seg) => {
            ...
            return (
              <div
                ...
                className={`bg-white rounded-lg border transition-all shadow-sm cursor-pointer ${isSelected
              ...
              >
                ...
                <div className={`p-3 border-b flex items-center justify-between ${isSelected ? 'bg-indigo-50 border-indigo-100' : 'bg-slate-50 border-slate-100'}`}>
                  ...
                  <div className="flex items-center gap-2">
                    <span className={`text-[10px] uppercase font-bold px-2 py-0.5 rounded-full ${seg.subject ? 'bg-blue-100 text-blue-700' : 'bg-slate-200 text-slate-500'
                      }`}>
                  ...
                  <div className="space-y-3" onClick={e => e.stopPropagation()}>
                    ...
                    <div className="pt-2 flex justify-end">
                      <button
                        ...
                        className={`text-xs px-3 py-1.5 rounded transition-colors flex items-center gap-1 border ${isSaved
                      ...
                      >
                        ...
                        {isSaved ? 'Saved' : isSaving ? 'Saving...' : 'Add to Bank'}
                      </button>
                    </div>
